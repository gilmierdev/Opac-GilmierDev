...
export function registerBooksIpc({ getDb, isAuthenticated }: Services): void {
  const ctx = { isAuthenticated }

  registerIpc(
    IPC.booksList,
    (filters: BookFilters = {}) => {
      return booksRepository(getDb()).list(sanitizeBookFilters(filters))
    },
    { context: ctx }
  )
  registerIpc(
    ...
    (id: number) => {
      ...
      return booksRepository(getDb()).getById(id)
    },
    { context: ctx }
  )
  registerIpc(
    ...
    (input: BookInput) => {
      ...
      return booksRepository(getDb()).create(input)
    },
    { context: ctx, requireAuth: true }
  )
  registerIpc(
    IPC.booksUpdate,
    (id: number, input: BookInput) => {
      if (!Number.isInteger(id) || id <= 0) throw new Error('Invalid book id')
      validateBookInput(input)
      return booksRepository(getDb()).update(id, input)
    },
    { context: ctx, requireAuth: true }
  )
  registerIpc(
    ...
    (id: number) => {
      if (!Number.isInteger(id) || id <= 0) throw new Error('Invalid book id')
      return booksRepository(getDb()).archive(id)
    },
    { context: ctx, requireAuth: true }
  )
  registerIpc(
    ...
    (id: number) => {
      if (!Number.isInteger(id) || id <= 0) throw new Error('Invalid book id')
      return booksRepository(getDb()).restore(id)
    },
    { context: ctx, requireAuth: true }
  )
  registerIpc(
    IPC.booksStats,
    () => booksRepository(getDb()).dashboardStats(),
    { context: ctx }
  )
}
